...
function Game(querySelector, options)
{
    ...
    this.draw = function() {
        ...
        apples.forEach(apple => {       //!!!!!!!!PROF IS HERE
            ctx.beginPath();
            ctx.fillStyle="#FF0000";
            ctx.fillRect(apple.x * gridSize, apple.y * gridSize, gridSize, gridSize);
            var profOfMath = new Image();
            profOfMath.onload = function(){
            ctx.drawImage(profOfMath, apple.x * gridSize, apple.y * gridSize);
        }
        profOfMath.src = 'images/teachers/beuchat_icon.jpg';
        ctx.closePath();
        });
